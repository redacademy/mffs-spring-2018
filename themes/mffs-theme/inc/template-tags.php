 ...
 */
  ...
  * @return bool
  */
 function mffs_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'mffs_categories' ) ) ) {
 		...
 		set_transient( 'mffs_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so mffs_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so mffs_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in mffs_categorized_blog.
  */
 function mffs_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'mffs_categories' );
 }
 add_action( 'edit_category', 'mffs_category_transient_flusher' );
 add_action( 'save_post',     'mffs_category_transient_flusher' );
  ...
  */
 function mffs_comment_list( $comment, $args, $depth ) {
  ...
  */
 function mffs_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
